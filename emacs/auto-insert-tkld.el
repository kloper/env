...
;; $Date: 2001/01/30 18:48:52 $|$Revision: 1.2 $|~/packages/auto-insert.tar.Z|
...
;; Revision 1.2  2001/01/30 18:48:52  dimka
;; changed to accept few extra tags
;;
...
(defconst auto-insert-version (substring "$Revision: 1.2 $" 11 -2)
  "$Id: auto-insert-tkld.el,v 1.2 2001/01/30 18:48:52 dimka Exp $
  ...
  (let* ((the-file-name (or buffer-file-name (buffer-name)))
	...
	(let* ((dirs (if auto-insert-search-current-dir
			 (cons default-directory auto-insert-path)
		       auto-insert-path))
	       (filedir (file-name-directory the-file-name))
	       (dir (concat filedir 
			    (file-relative-name (car dirs) filedir)))
	       (file (auto-insert-concat-file dir insert-file)))
	  (while (and (not (file-readable-p file)) (cdr dirs))
	    (setq dirs (cdr dirs))
	    (setq dir (concat filedir
			      (file-relative-name (car dirs) filedir)))
	    (setq file (auto-insert-concat-file dir insert-file)))
